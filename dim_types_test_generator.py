...
class DimTypesTestGenerator(ArrayTestGenerator):
    def _generate_programs(self):
        ...
        for dim_sizes_names in self.dim_sizes_names_set:
            ...
            for var_sizes_mask.vector in range(var_sizes_mask.num_system_base ** len(var_sizes)):
                ...
                A1 = Array('A1', Int, current_dim_sizes)
                ...
                dim_sizes_program = Program(
                    'mixed_dim_sizes_fun_arg_arr_enum_' + str(len(current_dim_sizes)) + 'd_' +
                ...
                dim_sizes_program = Program(
                    'mixed_dim_sizes_fun_arg_arr_const_int_' + str(len(current_dim_sizes)) +
                ...
                dim_sizes_program = Program(
                    'mixed_dim_sizes_fun_arg_arr_define_' + str(len(current_dim_sizes)) + 'd_' +
                    str(var_sizes_mask.vector))
                ...
                for i in range(len(var_sizes)):
                    if var_sizes_mask.get_bit(i):
                        ...
                        current_dim_sizes.append(array_sizes_enum[i])
                A1 = Array('A1', Int, *[array_sizes_enum[i] for i in range(len(var_sizes))])

                foo = Function('foo', Void, foo_arguments)
                foo.set_body(CodeBlock(pass_array(A1, ArrayLinearSubscriptTemplate(subscript_template_values))))

                dim_sizes_program = Program('mixed_dim_sizes_global_arr_' + str(len(current_dim_sizes)) + 'd_' +
                                            str(var_sizes_mask.vector))
                dim_sizes_program.add_enum(array_sizes_enum)
                dim_sizes_program.add_global_variable(A1)
                dim_sizes_program.add_function(foo)
                ...
                foo.set_body(CodeBlock(A1.get_declaration(),
                                       pass_array(A1, ArrayLinearSubscriptTemplate(subscript_template_values)),
                                       Return(A1)))

                dim_sizes_program = Program('mixed_dim_sizes_local_arr_' + str(len(current_dim_sizes)) + 'd_' +
                                            str(var_sizes_mask.vector))
                dim_sizes_program.add_enum(array_sizes_enum)
                dim_sizes_program.add_function(foo)
                write_arrays_to_json(dim_sizes_program.name, self.output_dir, A1)
