...
class SubscriptsC99WithStandardTestGenerator(ArrayTestGenerator):
    def _generate_programs(self):
        ...
        for dim_sizes_names in self.dim_sizes_names_set:
            ...
            standard_subscript_template = ArrayLinearSubscriptTemplate([(0, 1) for _ in range(len(dim_sizes_names))])
            standard_iter_vars = [Var('i' + str(i), Int) for i in range(len(dim_sizes_names))]
            ...
            for subscripts_mask.vector in range(subscripts_mask.num_system_base ** len(dim_sizes_names)):
                ...
                A1 = ArrayC99('A1', Int, array_sizes_enum[0], var_sizes[1],
                ...
                foo.set_body(
                    CodeBlock(Assignment(standard_subscript_template.generate_subscript(A1, standard_iter_vars), 20),
                              pass_array(A1, ArrayLinearSubscriptTemplate(subscript_template_values))))

                subscripts_program = Program('c99_mixed_sizes_mixed_subscripts_etalon_' + str(len(dim_sizes_names)) +
                                             'd_' + str(subscripts_mask.vector))
                ...
                A1 = ArrayC99('A1', Int, *[array_sizes_enum[i] for i in range(len(dim_sizes_names))])
                ...
                subscripts_program = Program('c99_enum_sizes_mixed_subscripts_etalon_' + str(len(dim_sizes_names)) +
                                             'd_' + str(subscripts_mask.vector))
                subscripts_program.add_enum(array_sizes_enum)
                subscripts_program.add_function(foo)
                write_arrays_to_json(subscripts_program.name, self.output_dir, A1, is_function_argument=True)
                ...
                A1 = ArrayC99('A1', Int, *[var_sizes[i] for i in range(len(dim_sizes_names))])
                ...
                subscripts_program = Program(
                    'c99_var_sizes_mixed_subscripts_etalon_' + str(len(dim_sizes_names)) + 'd_' +
